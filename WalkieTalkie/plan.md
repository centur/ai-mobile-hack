...
- Offline language selection prefers `SpeechTranscriber` and falls back to `DictationTranscriber` for broader on-device Dictation coverage. Locale variants are collapsed to one app-owned base-language value. The spoken-language selector opens a Speech model manager that lists every supported base language and provides download and reservation-release actions; the translated-to-language selector currently displays a "Not implemented" alert.
