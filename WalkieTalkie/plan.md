...
- Offline language selectors use the inventories required by each fixed panel role: the orange source reads installed `SpeechTranscriber` locales, while the blue target enumerates Translation languages whose source-to-target pair is installed according to `LanguageAvailability`. Locale variants are collapsed to one app-owned base-language value, and each menu includes a placeholder action for the future model-download flow.
