...
- Offline language selectors expose only pairs with installed `SpeechTranscriber` assets for both languages and installed Translation models in both directions according to `LanguageAvailability`. Locale variants are collapsed to one app-owned base-language value, and each menu includes a placeholder action for the future model-download flow.
