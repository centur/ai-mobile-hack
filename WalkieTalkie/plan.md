...
- `WalkieTalkie` now has a landscape-only, two-panel conversation UI. Either the orange bottom panel or blue top panel can become the speech source through its microphone, and translated text is displayed on the opposite panel. The blue panel occupies 5/8 of the available height; both panels have installed-language selectors.
...
- Offline language selection prefers `SpeechTranscriber` and falls back to `DictationTranscriber` for broader on-device Dictation coverage. Locale variants are collapsed to one app-owned base-language value. The bottom selector opens a Speech model manager that lists every supported base language and provides download and reservation-release actions; the top selector currently displays a "Not implemented" alert.
