...
- `WalkieTalkie` now has a landscape-only, two-panel conversation UI. Either the orange spoken-language panel or blue translated-to-language panel can initiate capture through its microphone, and translated text is displayed on the opposite panel. The translated-to panel occupies 5/8 of the available height.
...
- Both panel microphones use tap-to-toggle interaction. Tapping an idle microphone starts capture in that panel's language, and tapping it again finalizes the utterance. Tapping the opposite microphone finalizes an active utterance (or cancels setup if listening has not started) before transferring the single active recording role to the other panel. The active microphone pulses and emits an animated glow.
- Offline language selection prefers `SpeechTranscriber` and falls back to `DictationTranscriber` for broader on-device Dictation coverage. Locale variants are collapsed to one app-owned base-language value. The spoken-language selector opens a Speech model manager for individual assets. The translated-to-language selector opens a two-way readiness manager that prepares both Speech languages and Translation resources in both directions before marking a pair ready.
...
- The first speech backend slice is implemented: provider-neutral audio/transcript contracts, Apple microphone capture, on-device `SpeechAnalyzer`, preferred `SpeechTranscriber` with `DictationTranscriber` fallback, Speech asset inventory, status, download preparation, reservation release, format conversion, finalization, and cancellation.
...
- The provider-neutral voice pipeline emits original and translated text together. The UI discovers locally installed Speech assets, defaults the orange spoken language to the device's preferred installed language, prefers Indonesian (`id`) as the blue translated-to language when it differs, and checks the selected Translation pair through `LanguageAvailability`; final transcripts are translated only when that direction's offline model is installed.
- Physical-device speech/translation verification, interruption/route handling, test targets, synthesis, and persistence remain pending.
